    ...
    <div className="pt-24 pb-16 min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {userBookings.length === 0 ? (
        <div className="text-center py-20 card-cinema">
          ...
          <button onClick={() => router.push("/movies")} className="btn-gold mx-auto">
            Browse Movies
          </button>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`card-cinema overflow-hidden transition-all ${
                  isUpcoming ? "border-amber-500/30 shadow-[0_4px_20px_rgba(245,166,35,0.05)]" : "opacity-80"
                }`}
              >
                <div className="p-5">
                  ...
                  <div className="border-t border-cinema-border pt-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-bold text-amber-400">{formatCurrency(booking.totalAmount)}</p>
                    </div>
                    {booking.status === "confirmed" && (
                      <button
                        onClick={() => router.push(`/booking/confirmation?bookingId=${booking.id}`)}
                        className="text-xs px-3 py-1.5 bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 rounded-lg transition-colors"
                      >
                        View Ticket
                      </button>
                    )}
